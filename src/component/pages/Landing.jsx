            ...
            const checkStatus = async () => {
                try {
                    ...
                    if (res.data.isApproved) {
                        ...
                        await refreshUser();
                        toast.success("Congratulations! Your supplier account is now active.", {
                            icon: '🚀',
                            duration: 5000
                        });
                    }
        ...
        <div className="min-h-screen bg-white">
                            ...
                            {!user ? (
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    {activeView === 'seller' && !showAddForm && (
                                        <button onClick={() => setShowAddForm(true)} className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg font-bold transition-all flex items-center gap-2 shadow-sm">
                                    ...
                                    {user.role === 'seller' && user.isApproved && (
                                        <button 
                                            onClick={toggleView}
                                            className={`px-4 py-2 rounded-xl text-sm font-bold border transition-all ${activeView === 'seller' ? 'bg-orange-600 text-white border-orange-600 shadow-lg' : 'bg-white text-teal-600 border-teal-200 hover:bg-teal-50'}`}
                                        >
                                    ...
                                    {user.role === 'admin' && (
                                        <Link to="/admin-dashboard" className="text-teal-600 font-bold text-sm bg-teal-50 px-3 py-2 rounded-lg">Admin Panel</Link>
                                    )}
            ...
            <main className="min-h-[70vh]">
                {activeView === 'buyer' ? (
                    <>
                        <div className="w-full bg-gradient-to-r from-teal-50 to-blue-50 py-16">
                            <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-8 items-center">
                                <div>
                                    <h1 className="text-5xl font-extrabold text-gray-900 mb-4 leading-tight">Nepal's <span className="text-teal-600">Wholesale</span> Hub</h1>
                                    <p className="text-gray-600 text-lg mb-8">Connect with verified local suppliers. Bulk buying made simple.</p>
                                    {!user && <Link to="/register" className="bg-teal-600 text-white px-8 py-4 rounded-xl font-bold shadow-lg shadow-teal-100 hover:bg-teal-700 transition-all">Get Started Now</Link>}
                                </div>
                                <div className="hidden md:block">
                                    <img src="https://img.freepik.com/free-vector/isometric-e-commerce-concept_23-2148564680.jpg?w=740" alt="Wholesale" className="rounded-3xl shadow-2xl" />
                                </div>
                            </div>
                        </div>
                        <Categories />
                    ...
                    showAddForm ? <AddProductForm onBack={() => setShowAddForm(false)} /> : <SupplierProfile user={user} />
                )}
            </main>
